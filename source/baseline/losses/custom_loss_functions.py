...
class Margingw_MSEtc(LossWrapper):
    
    def __init__(self, always_apply=True, gw_criterion=None, variance_loss=False):
        ...
        self.mse_alpha = mse_alpha
        if gw_criterion == None:
            raise ValueError("For margin losses, it is better to provide the gw_criterion as network output depends on this.")
    ...
    def forward(self, outputs, targets, source_params, pe):
        # Margin-based loss for GW classification
        # MSE losses for PE
        custom_loss = {}
        
        ## Margin Loss for GW
        # NOTE: Margin value must be defined in the config file when defining GW criterion
        assert 'vector' in outputs.keys(), "key {vector} not present in output from network! Check model."
        positive = outputs['vector']['positive']
        negative = outputs['vector']['negative']
        
        if isinstance(self.gw_criterion, torch.nn.TripletMarginWithDistanceLoss):
            # For Triplet loss, anchor and positive must be from the same class
            # Negative must be the other class. The loss function tries to keep 'anchor'
            # and 'positive' close to each other and 'negative' as far away from 'anchor'
            # as possible. 
            ## NOTE: Distance function must be defined in config file if needed.
            # Positive vector is always reserved for sample with non-zero optimal SNR.
            # TripletLoss takes three inputs (a, p, n) -> anchor, positive and negative.
            ## NOTE: For training, we provide negative (noise class) as positive argument.
            ## This ensures that all noise samples are placed close to each other when the
            ## aanchor is also negative. Signals are placed farther apart based on their SNR.
            anchor = outputs['vector']['anchor']
            # torch.nn.TripletMarginWithDistanceLoss(anchor, positive, negative)
            # Since positive and negative are interchanged:
            margin_loss = self.gw_criterion(anchor, negative, positive)
        
        elif isinstance(self.gw_criterion, torch.nn.MarginRankingLoss):
            # For MarginRankingLoss, we only need positive and negative. 
            # It does not matter whether they represent signal or noise
            # Refer: https://pytorch.org/docs/stable/generated/torch.nn.MarginRankingLoss.html#torch.nn.MarginRankingLoss
            ...
            ## NOTE: Since positive is always chosen to be higher in optimal SNR than negative, target is always set to 1.
            ## Negative may be from the signal class or from a signal that has lower optimal SNR.
            margin_loss = self.gw_criterion(positive, negative, 1)

        custom_loss['gw'] = margin_loss
        ...
        for key in pe:
            ...
            if self.variance_loss:
                # Variance loss on PE terms
                assert key+"_var" in outputs.keys(), "Variance output not present for {} parameter!".format(key)
                pe_loss = torch.mean(((masked_target-masked_output)/(2.*outputs[key+'_var']))**2 - torch.log(outputs[key+'_var']))
            else:
                pe_loss = self.mse_alpha * torch.mean((masked_target-masked_output)**2)

            # Store losses
            try:
                if torch.isnan(pe_loss):
                    raise ValueError("PE Loss for {} is nan!".format(key))
            except:
                if np.isnan(pe_loss):
                    raise ValueError("PE Loss for {} is nan!".format(key))
        ...
        L = BCE(P_0) + alpha * MSE(P_1)
        """
        custom_loss['total_loss'] = margin_loss + mse_loss
