...
from architectures.zoo.resnet1d import ResNet1d
from architectures.zoo.resnet_cbam import resnet50_cbam
from architectures.zoo.res2net import res2net50, res2net101_26w_4s
from architectures.zoo.res2net_v1b import res2net101_v1b_26w_4s, res2net50_v1b_26w_4s
...
class KappaModel_Res2Net(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        tc_ = self.coalescence_time(out)
        tc = self.flatten_d0(tc_[:,0])
        norm_tc = self.Tanhshrink(tc)
        tc_var = self.Tanhshrink(self.flatten_d0(tc_[:,1]))
        # Chirp Distance
        dchirp_ = self.chirp_distance(out)
        dchirp = self.flatten_d0(dchirp_[:,0])
        norm_dchirp = self.sigmoid(dchirp)
        ...
        mchirp_ = self.chirp_mass(out)
        mchirp = self.flatten_d0(mchirp_[:,0])
        norm_mchirp = self.Tanhshrink(mchirp)
        mchirp_var = self.Tanhshrink(self.flatten_d0(mchirp_[:,1]))
        # Distance
        dist_ = self.distance(out)
        dist = self.flatten_d0(dist_[:,0])
        norm_dist = self.sigmoid(dist)
        ...
        snr_ = self.snr(out)
        snr = self.flatten_d0(snr_[:,0])
        norm_snr = self.Tanhshrink(snr)
        snr_var = self.Tanhshrink(self.flatten_d0(snr_[:,1]))
...
class KappaModel_Res2Net_singleDet(torch.nn.Module):
    """
    Kappa-type Model PE Architecture with Res2Net Block
    ...
    def __init__(self, 
        ...
        """ Frontend """
        # resnet50 --> Based on Res2Net blocks
        # Pretrained model is for 3-channels. We use 1 channel.
        # self.frontend = res2net50(pretrained=False)
        self.frontend_det1 = res2net50_v1b_26w_4s(pretrained=False, in_channels=1, num_classes=256)
        self.frontend_det2 = res2net50_v1b_26w_4s(pretrained=False, in_channels=1, num_classes=256)
        ...
        # Primary outputs
        self.signal_or_noise = nn.Linear(512, 1)
        self.coalescence_time = nn.Linear(256, 2)
        self.chirp_mass = nn.Linear(256, 2)
        # Mod layers
        self.signal_or_noise.to(dtype=data_type, device=self.store_device)
        self.coalescence_time.to(dtype=data_type, device=self.store_device)
        self.chirp_mass.to(dtype=data_type, device=self.store_device)
        ...
        self._det2.to(dtype=data_type, device=self.store_device)
        self.backend = {'det1': self._det1, 'det2': self._det2}
        self.frontend_det1.to(dtype=data_type, device=self.store_device)
        self.frontend_det2.to(dtype=data_type, device=self.store_device)
    ...
    def forward(self, x):
        ...
        # Timm Frontend
        out_H1 = self.frontend_det1(cnn_output_1) # (batch_size, 256)
        out_L1 = self.frontend_det2(cnn_output_2) # (batch_size, 256)
        out = torch.cat([out_H1, out_L1], dim=-1)
        out = self.flatten_d1(self.avg_pool_1d(out))
        ## Output necessary params
        raw = self.flatten_d0(self.signal_or_noise(out))
        pred_prob = self.sigmoid(raw)
        ...
        ## Parameter Estimation
        # Time of Coalescence
        tc_ = self.coalescence_time(out_H1)
        tc_1 = self.flatten_d0(tc_[:,0])
        norm_tc_1 = self.Tanhshrink(tc_1)
        tc_var_1 = self.Tanhshrink(self.flatten_d0(tc_[:,1]))

        tc_ = self.coalescence_time(out_L1)
        tc_2 = self.flatten_d0(tc_[:,0])
        norm_tc_2 = self.Tanhshrink(tc_2)
        tc_var_2 = self.Tanhshrink(self.flatten_d0(tc_[:,1]))

        # Chirp Mass
        mchirp_ = self.chirp_mass(out_H1)
        mchirp_1 = self.flatten_d0(mchirp_[:,0])
        norm_mchirp_1 = self.Tanhshrink(mchirp_1)
        mchirp_var_1 = self.Tanhshrink(self.flatten_d0(mchirp_[:,1]))
 
        mchirp_ = self.chirp_mass(out_L1)
        mchirp_2 = self.flatten_d0(mchirp_[:,0])
        norm_mchirp_2 = self.Tanhshrink(mchirp_2)
        mchirp_var_2 = self.Tanhshrink(self.flatten_d0(mchirp_[:,1]))
        
        # Return ouptut params (pred_prob, raw, cnn_output, pe_params)
        return {'raw': raw, 'pred_prob': pred_prob, 'cnn_output': cnn_output,
                'norm_tc_1': norm_tc_1, 'norm_tc_2': norm_tc_2,
                'norm_mchirp_1': norm_mchirp_1, 'norm_mchirp_2': norm_mchirp_2,
                'norm_tc_var_1': tc_var_1, 'norm_tc_var_2': tc_var_2,
                'norm_mchirp_var_1': mchirp_var_1, 'norm_mchirp_var_2': mchirp_var_2,
