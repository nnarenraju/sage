...
# LOCAL
from sage.data.generation.testdata_slicer import Slicer
from sage.data.generation.prior_modifications import (
    get_uniform_masses_with_mass1_gt_mass2,
    q_from_mass1_mass2,
    get_tau_priors,
    chirp_mass_from_signal_duration,
    mass1_mass2_from_mchirp_q,
)
from sage.data.generation.mlmdc_noise_generator import NoiseGenerator
from sage.data.generation.real_noise_datagen import RealNoiseGenerator
from sage.data.generation.generate_waveform import PyCBCGenerateWaveform
...
def get_complex_asds():
    ...
    psd_options = {
        "H1": [
            os.path.join(repo_abspath, "data/psds/H1/psd-{}.hdf".format(i))
            for i in range(20)
        ],
        "L1": [
            os.path.join(repo_abspath, "data/psds/L1/psd-{}.hdf".format(i))
            for i in range(20)
        ],
    }
...
class GenerateData:
    ...
    __slots__ = [
        ...
        "detectors_abbr",
        "noise_names",
        "save_injection_priors",
        "gc_collect_frequency",
        "hdf5_tree",
        "tmp",
        "num_queues_datasave",
        "num_cores_datagen",
        ...
        "whiten_padding",
        "groups",
        "error_padding_in_num",
        "sample_length_in_s",
        "sample_length_in_num",
        "waveform_kwargs",
        "noise_generator",
        "prior_low_mass",
        "prior_high_mass",
        "prior_low_chirp_dist",
        "prior_high_chirp_dist",
        "tc_inject_lower",
        "tc_inject_upper",
        "noise_high_freq_cutoff",
        "max_signal_length",
        "ringdown_leeway",
        "merger_leeway",
        "start_freq_factor",
        "fs_reduction_factor",
        "fbin_reduction_factor",
        "dbins",
        "check_seeds",
        "norm_tc",
        "norm_dist",
        ...
        "norm_invq",
        "complex_asds",
        "psd_est_segment_length",
        "psd_est_segment_stride",
        "blackout_max_ratio",
        "globtmp",
        "network_sample_length",
        "_decimated_bins",
        "corrupted_len",
        "mixed_noise",
        "mix_ratio",
    ...
    def __init__(self, **kwargs):
        ...
        if self.dataset == 1:
            ...
            self.psds = [
                psd_fun(self.psd_len, self.delta_f, self.noise_low_freq_cutoff)
        ...
        self.waveform_kwargs = {"delta_t": 1.0 / self.sample_rate}
        ...
        self.pycbc_wavegen.delta_t = 1.0 / self.sample_rate
        ...
        self.special["distrs"] = {}
        self.special["distrs"]["pol"] = distributions.angular.UniformAngle(
            uniform_angle=(0.0, 2.0 * np.pi)
        )
    ...
    def _save_(self, save_name, psd_data, psd_name):
        ...
        with h5py.File(psd_file_path, "a") as fp:
            ...
            key = "data"
            fp.create_dataset(
                key, data=data, compression="gzip", compression_opts=9, shuffle=True
            )
    ...
    def _MP_PSD_D4_(self, idx, slicer, sample_rate, ndet):
        ...
        O3a_real_noise_split = pycbc.types.TimeSeries(
            O3a_real_noise_split[ndet], delta_t=delta_t
        )
        ...
        estimated_psd = welch(
            O3a_real_noise_split, seg_len=seg_len, seg_stride=seg_stride
        )
    ...
    def save_PSD(self):
        ...
        if self.dataset == 1:
            ...
            psd_options = {
                "H1": [f"./data/psds/H1/psd-{i}.hdf" for i in range(20)],
                "L1": [f"./data/psds/L1/psd-{i}.hdf" for i in range(20)],
            }
            # Frequencies in the PSD
            f = np.linspace(
                self.noise_low_freq_cutoff, self.noise_high_freq_cutoff, self.psd_len
            )
            ...
            for det in self.detectors_abbr:
                ...
                blackout_idxs = np.argwhere(
                    ratio_psd > self.blackout_max_ratio
                ).flatten()
                ...
                print(
                    "Percentage of deleted bins while blacking out (D2,D3) = {}%".format(
                        del_ratio * 100
                    )
                )
                # Create blacked PSD
                ...
                ### Plotting
                plt.title(
                    "PSDs and their median for detector {} (removed = {}%)".format(
                        det, round(del_ratio * 100.0, 3)
                    )
                )
                plt.yscale("log")
                ...
                plt.plot(
                    ...
                    median_psd,
                    linewidth=3.0,
                    label="{} median PSD".format(det),
                    color="k",
                )
                for idx in blackout_idxs:
                    plt.plot(
                        [f[idx], f[idx]], [1e-49, 1e-38], linewidth=1.0, color="gray"
                    )
                plt.ylim(1e-49, 1e-38)
                plt.legend()
                save_png = os.path.join(
                    self.dirs["parent"], "median_PSDs_{}_full.png".format(det)
                )
            ...
            training_real_noise_file = os.path.join(
                noise_dir, "training_real_noise.hdf"
            )
            # Get the required attributes from real noise file
            with h5py.File(training_real_noise_file, "r") as fp:
                assert (
                    fp.attrs["dataset"] == 4
                ), "Wrong dataset accessed for D4 PSD calculation!"
                assert (
                    fp.attrs["mode"] == "training"
                ), "Data Leakage: File accessed is not in training mode!"
            ...
            assert (
                step_size <= training_duration
            ), "Step size chosen is larger than total O3a real noise training dataset!"
            ...
            with h5py.File(training_real_noise_file, "r") as infile:
                ...
                for ndet, det in enumerate(self.detectors_abbr):
                    ...
                    # Black out all regions where the ratio is greater than threshold
                    blackout_idxss = np.argwhere(
                        ratio_psds > self.blackout_max_ratio
                    ).flatten()
                    # Ratio is freq bins blacked out
                    del_ratios = len(blackout_idxss) / len(ratio_psds)
                    print(
                        "Percentage of deleted bins while blacking out (D4 - detector {}) = {}%".format(
                            det, del_ratios * 100
                        )
                    )
                    ...
                    blacked_psds[blackout_idxss] = (
                        999_999_999_999.0  # sqrt is about 1e6
                    )

                    ### Plotting
                    plt.title(
                        "PSDs and their median for detector {} (removed = {}%)".format(
                            det, round(del_ratios * 100.0, 3)
                        )
                    )
                    plt.yscale("log")
                    plt.xlabel("Frequency (Hz)")
    ...
    def get_priors(self):
        ...
        assert (
            float(config_reader["prior-tc"]["min-tc"]) == self.tc_inject_lower
        ), "min-tc in ini file {} different from tc_inject_lower {} in data_config".format(
            config_reader["prior-tc"]["min-tc"], self.tc_inject_lower
        )
        assert (
            float(config_reader["prior-tc"]["max-tc"]) == self.tc_inject_upper
        ), "max-tc in ini file {} different from tc_inject_upper {} in data_config".format(
            config_reader["prior-tc"]["max-tc"], self.tc_inject_upper
        )

        # Draw num_waveforms number of samples from ds.ini file
        priors = draw_samples_from_config(
            path=CONFIG_PATH, num=self.num_waveforms, seed=random_seed
        )

        ## MODIFICATIONS ##
        if self.modification[0] != None and isinstance(self.modification[0], str):
            # Check for known modifications
            if self.modification[0] in [
                "uniform_signal_duration",
                "uniform_chirp_mass",
            ]:
                _mass1, _mass2 = get_uniform_masses_with_mass1_gt_mass2(
                    self.prior_low_mass, self.prior_high_mass, self.num_waveforms
                )
            ...
            if self.modification[0] == "uniform_signal_duration":
                ...
                tau_lower, tau_upper = get_tau_priors(
                    self.prior_low_mass,
                    self.prior_high_mass,
                    self.signal_low_freq_cutoff,
                )
                mchirp = chirp_mass_from_signal_duration(
                    tau_lower,
                    tau_upper,
                    self.num_waveforms,
                    self.signal_low_freq_cutoff,
                ...
                raise ValueError(
                    "get_priors: Unknown modification added to data_cfg.modification!"
                )
            ...
            if self.modification[0] in [
                ...
                chirp_distance_distr = UniformRadius(
                    distance=(self.prior_low_chirp_dist, self.prior_high_chirp_dist)
                )
                dchirp = np.asarray(
                    [
                        chirp_distance_distr.rvs()[0][0]
                        for _ in range(self.num_waveforms)
                    ]
                )
            ...
            raise ValueError(
                "get_priors: Unknown data type used in data_cfg.modification!"
            )

        ## Get normalisation params for certain output values ##

        # Normalise time of coalescence
        self.norm_tc = Normalise(
            min_val=self.tc_inject_lower, max_val=self.tc_inject_upper
        )
        ...
        dist_lower = self.distance_from_chirp_distance(
            self.prior_low_chirp_dist, min_mchirp
        )
        dist_upper = self.distance_from_chirp_distance(
            self.prior_high_chirp_dist, max_mchirp
        )
        ...
        self.norm_dchirp = Normalise(
            min_val=self.prior_low_chirp_dist, max_val=self.prior_high_chirp_dist
        )
    ...
    def download_data(self, path, resume=True):
        ...
        if os.path.exists(path):
            ...
            print(
                "WARNING (contd.): Hide yo wives and get yo knives, a thicc boi is comin to town."
            )
    ...
    def optimise_fmin(self, h_pol):
        ...
        fmin = self.signal_low_freq_cutoff * (current_start_time / req_start_time) ** (
            3.0 / 8.0
        )
    ...
    def worker(self, idx, queues=None, qidx=None, save_idx=None):
        ...
        if not is_waveform:
            ...
            if self.dataset == 1:
                noise = [
                    self.noise_generator(psd).to_timeseries()[:maxlen]
                ...
                noise = self.noise_generator(
                    0.0, self.sample_length_in_s, self.sample_length_in_s
                )
            ...
            assert (
                len(noise[0]) == self.sample_length_in_num
            ), "Sample Length Error: Expected={}, Observed={}".format(
                self.sample_length_in_num, len(noise[0])
            )
            assert (
                len(noise[1]) == self.sample_length_in_num
            ), "Sample Length Error: Expected={}, Observed={}".format(
                self.sample_length_in_num, len(noise[1])
            )
            ...
            self.waveform_kwargs.update(
                dict(zip(prior_values.dtype.names, prior_values))
            )

            """ Injection """
            h_plus, h_cross, time_interval = self.pycbc_wavegen.apply(
                self.waveform_kwargs, self.special
            )
            start_interval, end_interval = time_interval

            assert (
                len(h_plus)
                == self.sample_length_in_num + self.error_padding_in_num * 2.0
            ), "Expected length = {}, actual length = {}".format(
                self.sample_length_in_num + self.error_padding_in_num * 2.0, len(h_plus)
            )
            assert (
                len(h_cross)
                == self.sample_length_in_num + self.error_padding_in_num * 2.0
            ...
            sample["norm_dchirp"] = self.norm_dchirp.norm(
                prior_values["chirp_distance"]
            )
            sample["norm_q"] = self.norm_q.norm(prior_values["q"])
            sample["norm_invq"] = 1.0 / sample["norm_q"]
        ...
        if self.gc_collect_frequency != -1:
            raise ValueError(
                "MP based dataset generation is not efficient with gc.collect()"
            )
    ...
    def _req_fields(self, names, max_nsamp, _group, _type):
        ...
        nonsample_names = [
            foo for foo in names if foo not in signal_names and foo not in noise_names
        ]
        dtype = np.float64
    ...
    def listener(self, queue, qidx, tmp=None):
        ...
        ** WARNING!!! **: If files are not being written, there should be an error in here.
        ...
        _ = [
            self._req_fields(self.waveform_names, self.max_nsamp_signal, grp, "signal")
            for grp in groups
        ]
        _ = [
            self._req_fields(self.noise_names, self.max_nsamp_noise, grp, "noise")
            for grp in groups
        ]
    ...
    def generate_dataset(self):
        ...
        watchers = [
            pool.apply_async(self.listener, (queue, n))
        ...
        save_qidxs = list(range(len(queues))) * int(
            sum(self.chunk_size) / self.num_queues_datasave
        )
        ...
        sidxs1 = np.array(
            [
                [n] * len(queues)
                for n in range(int(self.chunk_size[0] / self.num_queues_datasave))
            ]
        ).flatten()
        sidxs2 = np.array(
            [
                [n] * len(queues)
                for n in range(int(self.chunk_size[1] / self.num_queues_datasave))
            ]
        ).flatten()
        ...
        for job in pbar:
            ...
            pbar.set_description(
                "MPB DataGen: RAM curr={} MB, peak={} MB".format(curr_mem, max_mem)
            )
            job.get()
        ...
        _ = [
            self._req_fields(self.waveform_names, self.chunk_size[0], grp, "signal")
            for grp in groups
        ]
        _ = [
            self._req_fields(self.noise_names, self.chunk_size[1], grp, "noise")
            for grp in groups
        ]
    ...
    def plot_priors(self, save_dir):
        ...
        fig, ax = plt.subplots(
            num_rows, num_columns, figsize=(6.0 * num_columns, 3.0 * num_rows)
        )
        ...
        for dsetdir in dsetdirs:
            ...
            for nfile, chunk_file in enumerate(glob.glob(chunks)):
                ...
                with h5py.File(chunk_file, "r") as h5f:
                    ...
                    # Get all group tree
                    gtree = [
                        i + "/" + j for i, j in itertools.product(sr_grps, sn_grps)
                    ]
                    ...
                    for grp in gtree:
                        ...
                        for obj in h5f[grp].keys():
                            ...
                            main[extl_branch] = h5py.ExternalLink(
                                chunk_file, curr_branch
                            )
                        ...
                        tmp_var["path"].extend(
                            [foo + "/" + str(bar) for foo, bar in branch_paths]
                        )
            ...
            tmp_var_lens = [
                len(tmp_var[foo]) for foo in tmp_var.keys() if foo != "type"
            ]
            assert (
                len(list(set(tmp_var_lens))) == 1
            ), "var:tmp_var fields in extlinks have inconsistent column lengths!"
...
def _gen_(gd, mode=None, default_nums=None):
    ...
    for nchunk, chunk in enumerate(global_iterables):
        ...
        print("Time taken for chunk data generation = {} minutes".format(finish / 60.0))
...
def get_D4_noise(gd):
    ...
    real_noise_kwargs = dict(
        real_noise_path=noise_file,
        start_offset=0.0,
        duration=2592000.0,
        slide_buffer=240,
    ...
    step_size = (infile.attrs["duration"] - gd.sample_length_in_num) / (
        mix_ratio * gd.num_noises + 2000
    )
    ...
    assert (
        len(slicer) >= mix_ratio * gd.num_noises
    ), "Insufficient number ({}/{}) of samples in slicer object!".format(
        len(slicer), mix_ratio * gd.num_noises
    )
