...
# LOCAL
from sage.data.generation.segments import OverlapSegment
...
class RealNoiseGenerator:
    ...
    def __init__(self, **kwargs):
        ...
        print(
            "Testing dataset read. Contains {} days of noise".format(
                self.duration / (3600.0 * 24.0)
            )
        )
        ...
        with h5py.File(self.store_output[mode], "r") as infile:
            self.duration = infile.attrs["duration"]
        print(
            "Training dataset read. Contains {} days of noise".format(
                self.duration / (3600.0 * 24.0)
            )
        )
    ...
    def restrict_segments(self, mode=None):
        ...
        for seg in segments:
            ...
            if (
                ...
                > self.start_offset + self.duration
                    ...
                    past_duration
                    + segduration
                    - (self.start_offset + self.duration + self.slide_buffer)
                )
            ...
            if mode == "testing":
                ...
                if [start, end] in self.testing_segments:
                    continue
                if (
                    start == self.testing_segments[-1][0]
                    and end != self.testing_segments[-1][1]
                ):
        ...
        if past_duration < self.start_offset + self.duration:
            if mode == "training":
                ...
                return ret
            warnings.warn(
                "Not enough segments to generate the entire requested duration."
            )
    ...
    def get_real_noise(self, mode):
        ...
        with h5py.File(self.store_output[mode], "a") as fp:
            ...
            fp.attrs["sample_rate"] = 2048.0
            fp.attrs["low_frequency_cutoff"] = 15.0
            fp.attrs["filter_duration"] = 128.0
            fp.attrs["min_segment_duration"] = self.min_segment_duration
            fp.attrs["real_noise_path"] = (
                self.real_noise_path if self.real_noise_path is not None else "None"
            )
