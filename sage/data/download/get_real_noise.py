...
def downsample(strain, sample_rate=2048.0):
    res = resample_to_delta_t(strain, 1.0 / sample_rate)
    ret = highpass(res, low_freq_cutoff).astype(np.float32)
    ret = ret.time_slice(
        float(ret.start_time) + crop_after_decimation,
...
def fetcher(GPS_boundaries, num_workers=4, det="", run="", parent_dir=""):
    ...
    detector_data = []
    print(
        "Fetching GWOSC data for detector {} ({}) using {} cores".format(
            det, run, num_workers
        )
    )
        ...
        with mp.Pool(processes=num_workers) as pool:
            with tqdm(total=len(GPS_boundaries)) as pbar:
                ...
                for out in pool.imap_unordered(
                    get_detector_data,
                    [
                        (n, bound[0], bound[1], det)
                        for n, bound in enumerate(GPS_boundaries)
                    ],
                ):
                    n, data = out
                    if isinstance(data, np.ndarray):
                        with h5py.File(
                            os.path.join(
                                savedir_path,
                                "data_{}_{}_chunk_{}.hdf".format(det, run, n),
                            ),
                            "a",
                        ) as hf:
                            hf.create_dataset(
                                "data", data=data, compression="gzip", chunks=True
                            )
                    pbar.update()
    else:
        with tqdm(total=len(GPS_boundaries)) as pbar:
            ...
            for args in [
                (n, bound[0], bound[1], det) for n, bound in enumerate(GPS_boundaries)
            ]:
                n, data = get_detector_data(args)
                if isinstance(data, np.ndarray):
                    with h5py.File(
                        os.path.join(
                            savedir_path, "data_{}_{}_chunk_{}.hdf".format(det, run, n)
                        ),
                        "a",
                    ) as hf:
                        hf.create_dataset(
...
if __name__ == "__main__":
    ...
    ranges = {
        "O3a": {
            "H1": list(
                zip(H1_O3a_segments[:, 0], H1_O3a_segments[:, 1], H1_O3a_segments[:, 2])
            ),
            "L1": list(
                zip(L1_O3a_segments[:, 0], L1_O3a_segments[:, 1], L1_O3a_segments[:, 2])
            ),
            "V1": list(
                zip(V1_O3a_segments[:, 0], V1_O3a_segments[:, 1], V1_O3a_segments[:, 2])
            ),
        },
        "O3b": {
            "H1": list(
                zip(H1_O3b_segments[:, 0], H1_O3b_segments[:, 1], H1_O3b_segments[:, 2])
            ),
            "L1": list(
                zip(L1_O3b_segments[:, 0], L1_O3b_segments[:, 1], L1_O3b_segments[:, 2])
            ),
            "V1": list(
                zip(V1_O3b_segments[:, 0], V1_O3b_segments[:, 1], V1_O3b_segments[:, 2])
            ),
        },
    }
    ...
    for coord in itertools.product(["O3a", "O3b"], ["H1", "L1", "V1"]):
        ...
        print(
            "Total available duration in {} for {} = {}".format(
                det, run, available_valid_duration
            )
        )
        print(
            "Total valid duration in {} for {} = {}\n".format(
                det, run, total_valid_duration
            )
        )
        time.sleep(
            1
        )  # for some reason the print statements are omitted when using tqdm without this line. welp.

    # Get all data segments from H1 for O3a valid GPS times
    fetcher(
        valid_det_bounds["O3a"]["H1"],
        num_workers=8,
        det="H1",
        run="O3a",
        parent_dir="/data/wiay/nnarenraju/O3_SGWC_DATA",
    )

    # Get all data segments from H1 for O3a valid GPS times
    fetcher(
        valid_det_bounds["O3a"]["L1"],
        num_workers=8,
        det="L1",
        run="O3a",
        parent_dir="/data/wiay/nnarenraju/O3_SGWC_DATA",
