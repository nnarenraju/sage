...
def EradRational0815_s(eta, s, pv2const):
    eta2 = eta * eta
    eta3 = eta2 * eta
    eta4 = eta3 * eta
...
def EradRational0815(eta, chi1, chi2, pv2const):
    Seta = torch.sqrt(pv2const.ONE - 4.0 * eta)
    m1 = pv2const.HALF * (pv2const.ONE + Seta)
    m2 = pv2const.HALF * (pv2const.ONE - Seta)
    m1s = m1 * m1
    m2s = m2 * m2
    s = (m1s * chi1 + m2s * chi2) / (m1s + m2s)

    return EradRational0815_s(
        eta,
        s,
        pv2const,
    )
...
def FinalSpin0815_s(eta, S, pv2const):
    ...
    return eta * (
        ...
        + S
        * (
            (pv2const.ONE / eta - 0.0850917821418767 - 5.837029316602263 * eta)
...
def FinalSpin0815(eta, chi1, chi2, pv2const):
    # CL: Changed jnp to torch; expanded equation
    Seta = torch.sqrt(pv2const.ONE - 4.0 * eta)
    m1 = pv2const.HALF * (pv2const.ONE + Seta)
    m2 = pv2const.HALF * (pv2const.ONE - Seta)
    s = (m1 * m1) * chi1 + (m2 * m2) * chi2
    return FinalSpin0815_s(
        eta,
        s,
        pv2const,
    )
...
def convert_spins(
    ...
    A1 = pv2const.TWO + (3 * m2) / (2 * m1)
    A2 = pv2const.TWO + (3 * m1) / (2 * m2)
    ASp1 = A1 * S1_perp
    ASp2 = A2 * S2_perp
    ...
    m_sec = M * GM
    piM = torch.pi * m_sec
    v_ref = (piM * f_ref) ** pv2const.ONE_BY_THREE
    L0 = (
        M
        * M
        * L2PNR(
    ...
    tmp_x, tmp_y, tmp_z = pv2const.ZERO, pv2const.ZERO, pv2const.ONE
    tmp_x, tmp_y, tmp_z = ROTATEZ(-phiJ_sf, tmp_x, tmp_y, tmp_z)
    tmp_x, tmp_y, tmp_z = ROTATEY(-thetaJ_sf, tmp_x, tmp_y, tmp_z)
    ...
    PArunx_Jf = pv2const.ZERO
    PAruny_Jf = -pv2const.ONE
    PArunz_Jf = pv2const.ZERO

    # Q = NxP
    QArunx_Jf = Nz_Jf
    QAruny_Jf = pv2const.ZERO
    QArunz_Jf = -Nx_Jf
...
def SpinWeightedY(theta, phi, s, l, m, pv2const):
    ...
    if s == -2:
        if l == 2:
            if m == -2:
                fac = (
                    torch.sqrt(pv2const.FIVE / (64.0 * torch.pi))
                    * (1.0 - torch.cos(theta))
                    * (1.0 - torch.cos(theta))
                )
            elif m == -1:
                fac = (
                    torch.sqrt(pv2const.FIVE / (16.0 * torch.pi))
                    * torch.sin(theta)
                    * (1.0 - torch.cos(theta))
                )
            elif m == 0:
                fac = (
                    torch.sqrt(pv2const.FIFTEEN / (32.0 * torch.pi))
                    * torch.sin(theta)
                    * torch.sin(theta)
                )
            elif m == 1:
    ...
    return fac * torch.exp(pv2const.ONE_J * m * phi)


def L2PNR(v: torch.Tensor, eta: torch.Tensor, pv2const):
    ...
    return (
        ...
        * (
            pv2const.ONE
            + (pv2const.THREE_BY_TWO + eta / pv2const.SIX) * x
            + (3.375 - (19.0 * eta) / pv2const.EIGHT - eta2 / pv2const.TWENTY_FOUR) * x2
        )
    ) / torch.sqrt(x)


def WignerdCoefficients(
    v: torch.Tensor, SL: torch.Tensor, eta: torch.Tensor, Sp: torch.Tensor, pv2const
):
    ...
    s = Sp / (L + SL)
    s2 = s * s
    cos_beta = torch.sqrt(pv2const.ONE / (1.0 + s2))
    cos_beta_half = torch.sqrt(((1.0 + cos_beta) / pv2const.TWO))  # cos(beta/2)
    sin_beta_half = torch.sqrt(((1.0 - cos_beta) / pv2const.TWO))  # sin(beta/2)

    return cos_beta_half, sin_beta_half


def ComputeNNLOanglecoeffs(q, chil, chip, pv2const):
    m2 = q / (1.0 + q)
    m1 = pv2const.ONE / (1.0 + q)
    dm = m1 - m2
    mtot = pv2const.ONE
    eta = m1 * m2  # mtot = 1
    # This should prevent NaNs
...
def FinalSpin_inplane(
    ...
    af = torch.copysign(pv2const.ONE, af_parallel) * torch.sqrt(
...
def phP_get_fRD_fdamp(
    ...
    Erad = EradRational0815(
        eta_s,
        chi1_l,
        chi2_l,
        pv2const,
    )
    fRD = torch_linear_interp(finspin, pv2const.QNMData_a, pv2const.QNMData_fRD) / (
        1.0 - Erad
    )
    fdamp = torch_linear_interp(finspin, pv2const.QNMData_a, pv2const.QNMData_fdamp) / (
        1.0 - Erad
    )
...
def phP_get_transition_frequencies(
    ...
    f4_gammaneg_less_1 = torch.abs(
        f_RD
        + (f_damp * (-1 + torch.sqrt(pv2const.ONE - (gamma2) ** 2.0)) * gamma3) / gamma2
    )
