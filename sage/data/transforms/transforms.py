...
class UnifyNoiseGen:
    ...
    def __call__(self, special: dict):
        ...
        if special["training"]:
            ...
            hardsample = np.random.rand() < self.phard
...
class Resample(TransformWrapperPerChannel):
    def __init__(self, always_apply=True, factor=2.0):
        super().__init__(always_apply)
        self.factor = factor
...
class MonorateSampling(TransformWrapperPerChannel):
    def __init__(self, always_apply=True, sampling_rate=2048.0):
        super().__init__(always_apply)
...
class FastGenerateWaveform:
    ...
    def precompute_common_params(self):
        ...
        self.winlen = int(2.0 * (self.width / self.tmp_delta_f))
    ...
    def get_imr_duration(self, theta, f_lower):
        ...
        return 1.1 * lalsim.SimIMRPhenomDChirpTime(
            theta["mass1"] * 1.989e30,
            theta["mass2"] * 1.989e30,
            theta["spin1z"],
            theta["spin2z"],
            f_lower,
        )
    ...
    def optimise_fmin(self, theta):
        ...
        fudge_duration = (full_duration + 0.1 + self.rwrap) * self.duration_padfactor
    ...
    def convert_to_timeseries(self, hpol):
        ...
        return np.fft.irfft(hpol) * (1.0 / self.delta_t)

    def fd_taper_left(self, out):
        # Apply Tapering
        out[self.kmin : self.kmax] = (
            out[self.kmin : self.kmax] * self.window[: self.winlen // 2]
        )
        out[: self.kmin] = out[: self.kmin] * 0.0
        # Convert frequency series to time series
    ...
    def make_injection(self, hp, hc, params):
        ...
        right_pad = (
            int((end_time - sample_length_in_s) * self.sample_rate)
            if end_time > sample_length_in_s
            else 0
        )
    ...
    def project(self, hp, hc, special, params):
        ...
        strains = [
            det.project_wave(hp, hc, right_ascension, declination, pol_angle)
    ...
    def debug_waveform_generate(self, data, labels, special):
        ...
        save_path = os.path.join(
            cfg.export_dir, "DEBUG/waveform_generation/{}/{}".format(epoch, dirname)
        )
    ...
    def apply(self, params: dict, special: dict):
        ...
        if self.debug_me:
            self.debug_waveform_generate(
                data=[out[0], out[1]], labels=["H1", "L1"], special=special
            )
...
class CyclicShift(NoiseWrapper):
    ...
    def apply(self, y: np.ndarray, debug=""):
        ...
        augmented_noise = np.array(
            [np.roll(foo, num_roll[n]) for n, foo in enumerate(y)]
        )
...
class AugmentPhase(NoiseWrapper):
    ...
    def apply(self, y: np.ndarray, debug=""):
        ...
        dt = [
            random.randint(0, y.shape[1]) / 2048.0,
            random.randint(0, y.shape[1]) / 2048.0,
        ]
        ...
        augmented_noise_fft = [
            foo * np.exp(1j * dphi) for n, foo in enumerate(noise_fft)
        ]
...
class GlitchAugmentGWSPY:
    ...
    def __init__(
        ...
        assert (
            len(include) > 0
        ), "At least one observing run for a detector must be selected"
        ...
        self.num_glitches = {}
        for name in include:
            self.glitch_files[name] = [
                h5py.File(fname)
                for fname in glob.glob(os.path.join(glitch_dirs[name], "*.hdf"))
            ]
            self.num_glitches[name] = np.load(
                "/home/nnarenraju/Research/sgwc-1/sage/notebooks/tmp/{}.npy".format(
                    name
                )
            )
    ...
    def pick_glitch_file_H1(self, obs_run):
        ...
        H1opt = (
            [self.glitch_files["H1_O3a"], self.num_glitches["H1_O3a"]]
            if obs_run == "H1_O3a"
    ...
    def pick_glitch_file_L1(self, obs_run):
        ...
        # Glitch sample obtained from GWOSC using GWSPY GPS times
        L1opt = (
            [self.glitch_files["L1_O3a"], self.num_glitches["L1_O3a"]]
            if obs_run == "L1_O3a"
    ...
    def read_glitch(self, hf, length, data_cfg, is_training):
        ...
        start_time_ulimit = int(60.0 * 2048.0) - (
            int(data_cfg.sample_length_in_num) + recolour_pad
        )
        rand_start_idx = np.random.randint(low=0, high=start_time_ulimit)
        rand_slice = slice(
            rand_start_idx,
            int(rand_start_idx + data_cfg.sample_length_in_num + recolour_pad),
        )
    ...
    def apply(self, special):
        ...
        is_glitch = {"H1": np.random.rand() < 0.5, "L1": np.random.rand() < 0.5}
        ...
        pick_observing_run = {
            "H1": np.random.choice(h1_options),
        ...
        while True and is_glitch["H1"]:
            ...
            glitch_H1 = self.read_glitch(
                H1_file, H1_file_len, special["data_cfg"], special["training"]
            )
        ...
        while True and is_glitch["L1"]:
            ...
            glitch_L1 = self.read_glitch(
                L1_file, L1_file_len, special["data_cfg"], special["training"]
            )
        ...
        if self.debug_me:
            foo = "{}, {}, {}, {}".format(
                is_glitch["H1"],
                is_glitch["L1"],
                pick_observing_run["H1"],
                pick_observing_run["L1"],
            )
            self.tmp_debug.write(foo)
...
class RandomNoiseSlice:
    ...
    def precompute_common_params(self):
        ...
        lookup = np.load(
            "/home/nnarenraju/Research/sgwc-1/sage/notebooks/tmp/segdurs_all.npy"
        )
        for n, seg in enumerate(ligo_segments):
            ...
            if n >= self.segment_llimit and n <= self.segment_ulimit:
                ...
                seg_end_idx = segment_length - (
                    self.sample_length + self.segment_ends_buffer
                ) * (1.0 / self.dt)
    ...
    def _get_ligo_segments(self):
        ...
        for seg in segments:
            ...
            if (
                self.min_segment_duration is not None
                and segduration - self.slide_buffer < self.min_segment_duration
            ):
                continue
    ...
    def debug_random_noise_slice(self, data, labels, special):
        ...
        cfg = special["cfg"]
        save_path = os.path.join(
            cfg.export_dir, "DEBUG/noise_generation_d4/{}/{}".format(epoch, dirname)
        )
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        save = os.path.join(save_path, filename)
    ...
    def get_noise_segment(self, special, segdeets, det_only, recolour):
        ...
        for det, segdeet, seed in zip(self.detectors, segdeets, seeds):
            ...
            # This start time will lie within a valid segment time interval
            sample_start_idx = self._make_sample_start_time(
                seg_start_idx, seg_end_idx, seed
            )
            # Get the required portion of given segment
            sidx = sample_start_idx
        ...
        # Debugging
        if self.debug_me:
            self.debug_random_noise_slice(
                data=noise, labels=["H1 noise", "L1 noise"], special=special
            )
            dirname = "training" if special["training"] else "validation"
            epoch = special["epoch"]
            debug_args = (
