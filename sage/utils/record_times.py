...
def plot_split_times(ax, jobs, values, names, tot, errors=None):
    ...
    for name, val in zip(names, values):
        ...
        if name == "Avg Total Time (MP)":
            ...
            label = name + " ({}s, {}%)".format(
                round(val, 6), round((val / tot) * 100.0, 1)
            )
        else:
...
def record(plot_times, all_total_time, cfg):
    ...
    fig.suptitle(
        title.format(cfg.num_workers, cfg.batch_size, avg_ltime, avg_ttime), y=0.99
    )
    ...
    times_dict["Total Time"] = times_dict["Total Time"] - (
        sum(times_dict.values()) - times_dict["Total Time"]
    )
    plot_num = 513
    ...
    fmt_name = filename.format(
        cfg.batch_size, cfg.num_workers, str(datetime.date.today())
    )
