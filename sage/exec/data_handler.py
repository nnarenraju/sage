...
def configure_pipeline(opts):
    ...
    try:
        # Convert SageNetOTF -> sagenet_otf.py
        module_name = opts.config.lower()
        # Import sage.configs.sagenet_otf
        module = importlib.import_module(f"sage.configs.{module_name}")
        # Get the SageNetOTF class from the module
        cfg_class = getattr(module, opts.config)
        return cfg_class()
    except (ImportError, AttributeError) as e:
        raise ValueError(f"Could not load config '{opts.config}'. "
                       f"Make sure sage/configs/{module_name}.py exists and "
                       f"contains class {opts.config}")

def configure_dataset(opts):
    ## Read dataset configuration
    try:
        module_name = opts.data_config.lower()
        module = importlib.import_module(f"sage.configs.{module_name}")
        data_cfg_class = getattr(module, opts.data_config)
        data_cfg = data_cfg_class()
        
        # Rest of your existing code for ini file configuration...
        ...
        raise ValueError(f"Could not load data config '{opts.data_config}'. "
                       f"Make sure sage/configs/{module_name}.py exists and "
                       f"contains class {opts.data_config}")
...
def _cfg_to_ini_():
    ...
    cfg_to_ini["prior_low_mass"] = [
        ("min-mass1", "prior-mass1"),
        ("min-mass2", "prior-mass2"),
    ]
    cfg_to_ini["prior_high_mass"] = [
        ("max-mass1", "prior-mass1"),
        ("max-mass2", "prior-mass2"),
    ]
    cfg_to_ini["prior_low_chirp_dist"] = [
        ("min-chirp_distance", "prior-chirp_distance")
    ]
    cfg_to_ini["prior_high_chirp_dist"] = [
        ("max-chirp_distance", "prior-chirp_distance")
    ]
    return cfg_to_ini
...
class DataModule:
    ...
    def configure_dataset(opts):
        ...
        cfg_to_ini = _cfg_to_ini_()
        config = configparser.ConfigParser()
        ini_template_path = "/home/nnarenraju/Research/sage/repro/ini_files/templates/ds{}_template.ini".format(
            data_cfg.dataset
        )
        ini_file_path = (
            "/home/nnarenraju/Research/sage/repro/ini_files/ds{}.ini".format(
                data_cfg.dataset
            )
        )
        ...
        if not data_cfg.OTF:
            ...
            if os.path.isdir(check_dir):
                ...
                raise FileNotFoundError(
                    f"prepare_data.get_summary: {check_dir} not found!"
                )
        else:
    ...
    def get_metadata(cfg, data_cfg):
        ...
        train = pd.DataFrame(
            lookup, columns=["id", "path", "target", "dstype", "mchirp"]
        )
        ...
        if cfg.debug:
            ...
            if data_cfg.dataset in [1, 2, 3]:
                ...
                training_data = train.loc[train["dstype"] == "training"].head(
                    num_training
                )
                balance_params = {"mchirp": training_data["mchirp"].values}
                validation_data = train.loc[train["dstype"] == "validation"].head(
                    num_validation
                )
        ...
        if data_cfg.dataset in [1, 2, 3]:
            X_train, X_valid, _, _ = train_test_split(
                idxs, all_targets, test_size=0.2, random_state=42, stratify=all_targets
            )
    ...
    def get_dataset_objects(cfg, data_cfg, train_fold, valid_fold):
        ...
        if not data_cfg.OTF:
            ...
            if check_class_balance != 0.5 and not cfg.ignore_dset_imbalance:
                raise ValueError(
                    "Encountered a class imbalanced (num_signals/tot = {}) training dataset!".format(
                        check_class_balance
                    )
                )
        ...
        train_dataset = cfg.dataset(
            ...
            targets=targets,
            waveform_generation=cfg.generation["signal"],
            noise_generation=cfg.generation["noise"],
            transforms=cfg.transforms["train"],
            target_transforms=cfg.transforms["target"],
        ...
        if not data_cfg.OTF:
            ...
            if check_class_balance != 0.5 and not cfg.ignore_dset_imbalance:
                raise ValueError(
                    "Encountered a class imbalanced (num_signals/tot = {}) validation dataset!".format(
                        check_class_balance
                    )
                )
    ...
    def input_sample_length(data_cfg):
        ...
        for sidx, eidx, fs in data_cfg.dbins:
            df = decimation_factor(2048.0, fs)
        ...
        if isinstance(data_cfg.corrupted_len, int):
            input_length -= 2.0 * data_cfg.corrupted_len
        ...
        _, data_cfg._decimated_bins = multirate_sampling(
            dummy_data, data_cfg, check=True
        )
