        ...
        noise=UnifyNoiseGen(
            {
                "training": ColouredNoiseGenerator(
                    psds_dir=os.path.join(repo_abspath, "data/psds")
                ),
        ...
        noise=UnifyNoise(
            [
                Recolour(
                    use_precomputed=True,
                    h1_psds_hdf=os.path.join(
                        repo_abspath, "notebooks/tmp/psds_H1_latter51days_20s.hdf"
                    ),
                    l1_psds_hdf=os.path.join(
                        repo_abspath, "notebooks/tmp/psds_L1_latter51days_20s.hdf"
                    ),
                    p_recolour=0.3829,
                    debug_me=False,
                    debug_dir=os.path.join(debug_dir, "Recolour"),
...
class SageNetOTF_Feb05_Russet_bayes_factor(SageNetOTF):
    ...
    weights_path = "/home/nnarenraju/Research/sgwc-1/RUNS/SageNet50_bayes_factor_Feb06_Russet/BEST/weights_low_far_nsignals_39.pt"
...
class Validate_1epoch_TrainRecolour(SageNetOTF):
    ...
    signal_param = {
        ...
        "f_ref": 20.0,
        "mass1": 34.78719347340714,
        "mass2": 26.213305061289596,
        "ra": 3.1059617471553547,
        "dec": -0.6182337605590603,
        "inclination": 0.8318006348684067,
        "coa_phase": 1.7465934740664464,
        "polarization": 2.8848538945232653,
        "chirp_distance": 240.66621748823462,
        "spin1_a": 0.3205292059008806,
        "spin1_azimuthal": 2.791139350243793,
        "spin1_polar": 1.5426111575393548,
        "spin2_a": 0.3809497458109482,
        "spin2_azimuthal": 5.555278986183884,
        "spin2_polar": 1.8106375482871242,
        "injection_time": 1242017656.1400197,
        "tc": 11.085430996548288,
        "spin1x": -0.3009269684153754,
        "spin1y": 0.11000153135040237,
        "spin1z": 0.009032973837402562,
        "spin2x": 0.2762643625638985,
        "spin2y": -0.24619412379548633,
        "spin2z": -0.09049400101200968,
        "mchirp": 26.236024494682457,
        "q": 1.3270815485521894,
        "distance": 3106.193154419415,
    }
        ...
        train=Unify(
            {
                "stage1": [
                    Whiten(
                        trunc_method="hann", remove_corrupted=False, estimated=False
                    ),
                ],
                "stage2": [
                    Normalise(ignore_factors=True),
...
class SageNetOTF_Feb18_Russet_Broad(SageNetOTF):
    """Data storage"""

    name = "SageNet50_BEST_Feb18_Russet_broader_mass"
    export_dir = Path("/home/nnarenraju/Research/ORChiD/RUNS") / name
    debug_dir = "./DEBUG"
    git_revparse = subprocess.run(
        ["git", "rev-parse", "--show-toplevel"], capture_output=True, text=True
    ...
    testing_dir = "/home/nnarenraju/Research/ORChiD/test_data_d4"
    test_foreground_output = "testing_foutput_BEST_broader_mass.hdf"
    test_background_output = "testing_boutput_BEST_broader_mass.hdf"
