...
def display_outputs(training_output, training_labels):
    ...
    for out, lab in zip(training_output, training_labels):
        print(
            "output = {}, label = {}".format(out.detach().numpy(), lab.detach().numpy())
        )
...
def prediction_raw(nep, output, labels, export_dir):
    ...
    _plot(
        ...
        y=raw_tp,
        label="Signals",
        ylabel="log10 Number of Occurences",
        xlabel="Raw Output Values",
        yscale="log",
        histogram=True,
    )
    _plot(
    ...
    _plot(
        ...
        x=count_curve[:, 0],
        y=count_curve[:, 1],
        xlabel="Noise Stat Threshold",
        ylabel="Frac Signals Detected above Threshold",
        ls="solid",
        c="red",
        yscale="linear",
        xscale="linear",
        histogram=False,
        label="1FAR/x = {}/{}".format(
            int(count_curve[:, 1][0] * len(raw_tp)), len(raw_tp)
        ),
    )
...
def prediction_probability(nep, output, labels, export_dir):
    ...
    _plot(
        ...
        c="red",
        ylabel="log10 Number of Occurences",
        xlabel="Prediction Probabilities (Sigmoid)",
        yscale="log",
        histogram=True,
    )
    _plot(
...
def efficiency_curves(
    ...
    for key in source_params.keys():
        ...
        for idx in overlap_range:
            bdata = np.where(
                (zsorted[:, 0] > zsorted[:, 1][idx])
                & (zsorted[:, 0] < zsorted[:, 1][idx + bin_width])
            )[0]
        ...
        save_path = os.path.join(
            save_dir, "efficiency_{}_{}_{}.png".format(save_name, key, nep)
        )
        plt.savefig(save_path)
        plt.close()


def learning_parameter_prior(
    nep,
    source_params,
    predicted_outputs,
    labels,
    ...
    for key in source_params.keys():
        ...
        overlap_range = range(0, len(zsorted), overlap)
        plot = [
            [zsorted[:, 0][idx], np.mean(zsorted[:, 1][idx : idx + bin_width])]
            for idx in overlap_range
        ]
        plot = np.array(plot)
        # Plotting the above data for the given parameter
        ...
        plt.fill_between(
            plot[:, 0],
            plot[:, 1] - np.percentile(plot[:, 1], 95),
            plot[:, 1] + np.percentile(plot[:, 1], 95),
            color="red",
            alpha=0.3,
        ...
        plt.fill_between(
            plot[:, 0],
            plot[:, 1] - np.percentile(plot[:, 1], 50),
            plot[:, 1] + np.percentile(plot[:, 1], 50),
            color="green",
            alpha=0.3,
        ...
        # Saving the plot in export_dir
        save_path = os.path.join(
            save_dir, "learning_{}_{}_{}.png".format(save_name, key, nep)
        )
        plt.savefig(save_path)
        plt.close()
...
def diagonal_compare(nep, outputs, labels, network_snrs, export_dir):
    ...
    for param in outputs.keys():
        ...
        ax_snr_gt8, fig_gt8 = figure(
            title="Diagonal Plot of {} (SNR>8) at Epoch = {}".format(param, nep)
        )
        ...
        plot_output_gt8, plot_labels_gt8, plot_snrs_gt8 = np.hsplit(
            data_gt8, data_gt8.shape[1]
        )
        # Plotting
        foo = ax.scatter(
            plot_output, plot_labels, marker=".", s=200.0, c=plot_snrs, cmap=cmap
        )
        bar = ax_snr_gt8.scatter(
            plot_output_gt8,
            plot_labels_gt8,
            marker=".",
            s=200.0,
        ...
        if param != "norm_dist" and param != "norm_dchirp":
            _plot(
                ...
                xlabel="Observed Value [{}]".format(param),
                ls="dashed",
            )
        ...
        save_path_gt8 = os.path.join(
            save_dir, "diagonal_snr_gt8_{}_{}.png".format(param, nep)
        )
...
def outputbin_param_distribution(
    cfg, export_dir, network_output, labels, sample_params, epoch
):
    ...
    bin_ranges = [
        (min(signal_stats), 0.0),
        (min(signal_stats), max(noise_stats)),
        (0.0, max(noise_stats)),
        (max(noise_stats), max(signal_stats)),
    ...
    bin_names = [
        "(min signal stat to 0.0)",
        "(min signal stat to max noise stat)",
        "(0.0 to max noise stat)",
        "(max noise stat to max signal stat)",
    ...
    for n, bin_range in enumerate(bin_ranges):
        ...
        idxs = np.argwhere(
            (signal_stats > bin_range[0]) & (signal_stats < bin_range[1])
        ).flatten()
        # Histogram details
        ...
        _plot(
            ...
            label="Signals",
            ylabel="log10 Number of Occurences",
            xlabel="Network Raw Output Values",
            yscale="log",
            histogram=True,
        )
        ...
        plt.axvline(
            ...
            ls=":",
            lw=3,
            label="lower bin edge = {}".format(np.round(bin_range[0], 2)),
        )
        plt.axvline(
            x=bin_range[1],
            color="k",
            ls="--",
            lw=3,
            label="upper bin edge = {}".format(np.round(bin_range[1], 2)),
        )
        # Save
        plt.legend()
...
def paramfrac_detected_above_thresh(
    cfg, export_dir, network_output, labels, sample_params, epoch
):
    ...
    for param, distr in sample_params.items():
        ...
        fig, ax = plt.subplots(1, 2, figsize=(12.0 * 2, 8.0 * 1))
        plt.suptitle(
            "Epoch {}: Fraction of binned {} above noise thresholds".format(
                epoch, param
            )
        )
        ...
        _plot(
            ...
            xlabel="{} (epoch {})".format(param, epoch),
            yscale="linear",
            histogram=True,
        )
        ...
        for n in range(len(bin_edges) - 1):
            ...
            idxs = np.argwhere(
                (masked_distr > bin_range[0]) & (masked_distr < bin_range[1])
            ).flatten()
            ...
            for thresh in sorted_noise_stats[::-1]:
                count_curve.append(
                    [
                        thresh,
                        len(binmasked_stats[binmasked_stats > thresh])
                        / len(binmasked_stats),
                    ]
                )
            # convert to numpy
            ...
            _plot(
                ...
                xscale="linear",
                histogram=False,
                label="{} to {}, 1FAR/x = {}/{}".format(
                    np.round(bin_range[0], 2),
                    np.round(bin_range[1], 2),
                    int(count_curve[:, 1][0] * len(binmasked_distr)),
                    len(binmasked_distr),
                ),
            )
...
def loss_and_accuracy_curves(cfg, filepath, export_dir, best_epoch=-1):
    ...
    if best_epoch != -1:
        ax.scatter(
            epochs[best_epoch], training_loss[best_epoch], marker="*", s=200.0, c="k"
        )
        ax.scatter(
            epochs[best_epoch], validation_loss[best_epoch], marker="*", s=200.0, c="k"
        )
    plt.legend()
    ...
    if "parameter_estimation" in cfg.model_params.keys():
        if len(cfg.model_params["parameter_estimation"]) != 0:
            tsearch_names = [
                foo + "_tloss"
                for foo in cfg.model_params["parameter_estimation"] + ("gw",)
            ]
            vsearch_names = [
                foo + "_vloss"
                for foo in cfg.model_params["parameter_estimation"] + ("gw",)
            ...
            for n, (tsearch_name, vsearch_name) in enumerate(
                zip(tsearch_names, vsearch_names)
            ):
...
def batchshuffle_noise(training_labels, training_samples, nbatch, nep):
    ...
    if len(noise_idx) > 0:
        ...
        assert (
            seed1 != seed2
        ), "Noise idx have not been shuffled properly. Seeds are the same for detectors!"
        ...
        for bidx in range(training_samples.shape[0]):
            if bidx in noise_idx:
                d1, d2 = shuffler[bidx]
                shuffled_training_samples.append(
                    [training_samples[d1][0].numpy(), training_samples[d2][1].numpy()]
                )
            else:
                shuffled_training_samples.append(
                    [
                        training_samples[bidx][0].numpy(),
                        training_samples[bidx][1].numpy(),
                    ]
                )
...
def consolidated_display(train_time, total_time):
    ...
    print("\n----------------------------------------------------------------------")
    print(
        "Total time taken to train epoch = {}s".format(np.around(np.sum(train_time), 3))
    )
    print(
        "Average time taken to train per batch = {}s".format(
            np.around(np.mean(train_time), 3)
        )
    )
    # Calculate MP load time
    load_time = total_time - np.sum(train_time)
    print(
        "Total time taken to load (read & preprocess) epoch = {}s".format(
            np.around(load_time, 3)
        )
    )
...
def training_phase(
    ...
    accuracy = calculate_accuracy(
        training_output["pred_prob"], training_labels["gw"], 0.5
    )
...
def validation_phase(
    ...
    with torch.cuda.amp.autocast() if cfg.do_AMP else nullcontext():
        ...
        with torch.no_grad():
            ...
            all_losses = loss_function(
                validation_output, validation_labels, source_params, cfg
            )

    # Accuracy calculation
    accuracy = calculate_accuracy(
        validation_output["pred_prob"], validation_labels["gw"], 0.5
    )
...
def train(
    ...
    try:
        ...
        best_loss = (
            checkpoint["loss"] if cfg.resume_from_checkpoint else 1.0e10
        )  # impossibly bad value
        ...
        best_params = {
            ...
            "low_far_nsignals": best_low_far_nsignals,
        }
        ...
        for nep in range(start_epoch, cfg.num_epochs):
            ...
            if "parameter_estimation" in cfg.model_params.keys():
                training_running_loss = dict(
                    zip(
                        cfg.model_params["parameter_estimation"],
                        [0.0] * len(cfg.model_params["parameter_estimation"]),
                    )
                )
            else:
            ...
            if not validate_1epoch:
                ...
                for nbatch, (
                    ...
                    if cfg.batchshuffle_noise:
                        training_samples = batchshuffle_noise(
                            training_labels, training_samples, nbatch, nep
                        )
                    ...
                    training_samples = training_samples.to(
                        dtype=torch.float32, device=cfg.train_device
                    )
                    for key, value in training_labels.items():
                        training_labels[key] = value.to(
                            dtype=torch.float32, device=cfg.train_device
                        )

                    """ Call Training Phase """
                    # Run training phase and get loss and accuracy
                    training_loss, accuracy = training_phase(
                        nep,
                        cfg,
                        data_cfg,
                        Network,
                    ...
                    ## Display stuff
                    loss = np.around(
                        training_loss["total_loss"].clone().cpu().item(), 8
                    )
                    # Update pbar with loss, acc
                    pbar.set_description(
                        "Epoch {}, batch {} - loss = {}".format(
                            nep, training_batches, loss
                        )
                    )
                    ...
                    for key in training_running_loss.keys():
                        training_running_loss[key] += (
                            training_loss[key].clone().cpu().item()
                        )
            ...
            with torch.no_grad():
                ...
                for nbatch, (
                    ...
                    """Set the device and dtype"""
                    validation_samples = validation_samples.to(
                        dtype=torch.float32, device=cfg.train_device
                    )
                    for key, value in validation_labels.items():
                        validation_labels[key] = value.to(
                            dtype=torch.float32, device=cfg.train_device
                        )

                    """ Call Validation Phase """
                    # Run training phase and get loss and accuracy
                    validation_loss, accuracy, voutput = validation_phase(
                        nep,
                        cfg,
                        data_cfg,
                        Network,
                    ...
                    # Display stuff
                    loss = np.around(
                        validation_loss["total_loss"].clone().cpu().item(), 8
                    )
                    pbar.set_description(
                        "Epoch {}, batch {} - loss = {}".format(
                            nep, validation_batches, loss
                        )
                    )
                    ...
                    for key in validation_running_loss.keys():
                        validation_running_loss[key] += (
                            validation_loss[key].clone().cpu().item()
                        )
                    ...
                    if nep % cfg.validation_plot_freq == 0:
                        ...
                        epoch_labels["gw"] = np.concatenate(
                            [epoch_labels["gw"], validation_labels["gw"].cpu()]
                        )
                        epoch_outputs["gw"] = np.concatenate(
                            [epoch_outputs["gw"], voutput["pred_prob"].cpu()]
                        )
                        ...
                        if len(pe_params) != 0:
                            for param in pe_params:
                                epoch_labels[param] = np.concatenate(
                                    [
                                        epoch_labels[param],
                                        validation_labels[param].cpu(),
                                    ]
                                )
                                epoch_outputs[param] = np.concatenate(
                                    [epoch_outputs[param], voutput[param].cpu()]
                                )

                # Convert source params into dictionary of np.ndarray
                sample_params = dict(
                    (key, np.concatenate(tuple(val))) for key, val in dd.items()
                )

                if nep % cfg.validation_plot_freq == 0:

                    """ROC Curve save data"""
                    roc_auc, fpr, tpr = roc_curve(
                        nep, epoch_outputs["gw"], epoch_labels["gw"], export_dir
                    )

                    """ Prediction Probabilitiy OR Raw Value histograms  """
                    low_far_nsignals = prediction_raw(
                        nep, raw_output, epoch_labels["gw"], export_dir
                    )
                    ...
                    diagonal_compare(
                        ...
                        epoch_outputs,
                        epoch_labels,
                        sample_params["network_snr"],
                        export_dir,
                    )

                    """ Param distribution of network raw output bins """
                    outputbin_param_distribution(
                        cfg,
                        export_dir,
                        raw_output,
                        epoch_labels["gw"],
                    ...
                    """ Param Fraction Plot """
                    paramfrac_detected_above_thresh(
                        cfg,
                        export_dir,
                        raw_output,
                        epoch_labels["gw"],
            ...
            if validate_1epoch:
                with h5py.File(
                    os.path.join(export_dir, "validation_output.hdf"), "a"
                ) as ds:
                    # Raw output
                    ds.create_dataset(
                        "raw_output",
            ...
            if validate_1epoch:
                ...
                with torch.no_grad():
                    ...
                    for cf in range(4):
                        ...
                        for nbatch, (
                            ...
                            # Set the device and dtype
                            aux_samples = aux_samples.to(
                                dtype=torch.float32, device=cfg.train_device
                            )
                            for key, value in aux_labels.items():
                                aux_labels[key] = value.to(
                                    dtype=torch.float32, device=cfg.train_device
                                )

                            # Call Validation Phase
                            # Run training phase and get loss and accuracy
                            aux_loss, accuracy, _ = validation_phase(
                                nep,
                                cfg,
                                data_cfg,
                                Network,
                            ...
                            # Display stuff
                            loss = np.around(
                                aux_loss["total_loss"].clone().cpu().item(), 8
                            )
                            pbar.set_description(
                                "AUX Epoch {}, batch {} - loss = {}".format(
                                    nep, aux_val_batches, loss
                                )
                            )

                            # Update losses and accuracy
                            aux_val_batches += 1
                            aux_val_running_loss["aux_{}".format(cf)] += (
                                aux_loss["gw"].clone().cpu().item()
                            )
                ...
                np.save("./aux_losses_{}.npy".format(cfg.name), aux_losses)
            ...
            if cfg.epoch_testing:
                ...
                output_testing_dir = os.path.join(
                    epoch_testing_dir, "epoch_testing_{}".format(nep)
                )
                for job in jobs:
                    # Get the required data based on testing job
                    if job == "foreground":
                        testfile = os.path.join(
                            cfg.epoch_testing_dir, cfg.test_foreground_dataset
                        )
                        evalfile = os.path.join(
                            output_testing_dir, cfg.test_foreground_output
                        )
                    elif job == "background":
                        testfile = os.path.join(
                            cfg.epoch_testing_dir, cfg.test_background_dataset
                        )
                        evalfile = os.path.join(
                            output_testing_dir, cfg.test_background_output
                        )
                ...
                raw_args = [
                    "--injection-file",
                    os.path.join(cfg.epoch_testing_dir, cfg.injection_file),
                ]
                raw_args += [
                    "--foreground-events",
                ...
                raw_args += [
                    "--foreground-files",
                    os.path.join(cfg.epoch_testing_dir, cfg.test_foreground_dataset),
                ]
                raw_args += [
                    "--background-events",
            ...
            for key in training_running_loss.keys():
                ...
                epoch_validation_loss[_key] = np.around(
                    validation_running_loss[key] / validation_batches, 8
                )
                epoch_training_loss[_key] = np.around(
                    training_running_loss[key] / training_batches, 8
                )
            ...
            with open(loss_filepath, "a") as outfile:
                ...
                if add_fieldnames:
                    ...
                    all_fields = (
                        epoch_field + tloss_fields + vloss_fields + other_fields
                    )
            ...
            if cfg.save_checkpoint and nep % cfg.checkpoint_freq == 0:
                ...
                checkpoint_save_path = os.path.join(
                    save_dir, "checkpoint_epoch_{}.pt".format(nep)
                )
                torch.save(
                    {
                        "epoch": nep,
            ...
            epoch_params = {
                "loss": epoch_validation_loss["gw"],
                "accuracy": avg_acc_valid,
                "roc_auc": roc_auc,
                "low_far_nsignals": low_far_nsignals,
            }
            param_operator = {
                "loss": operator.lt,
                "accuracy": operator.gt,
                "roc_auc": operator.gt,
                "low_far_nsignals": operator.gt,
            }

            for wtype in cfg.weight_types:
                if (
                    param_operator[wtype](epoch_params[wtype], best_params[wtype])
                    and wtype in cfg.weight_types
                ):
                    weights_path = "{}_{}{}".format(
                        weights_root_name, wtype, weights_file_ext
                    )
            ...
            if nep in cfg.save_epoch_weight:
                weights_path = "{}_{}_{}{}".format(
                    weights_root_name, wtype, nep, weights_file_ext
                )
                weights_save_path = os.path.join(export_dir, weights_path)
            ...
            """ Epoch Display """
            print(
                "\nBest Validation Loss (wrt all past epochs) = {}".format(
                    best_params["loss"]
                )
            )
            ...
            if "parameter_estimation" in cfg.model_params.keys():
                for n, param in enumerate(cfg.model_params["parameter_estimation"]):
                    print(
                        "{}. {} Loss = {}".format(
                            n + 2, param, epoch_validation_loss[param]
                        )
                    )
            ...
            if "parameter_estimation" in cfg.model_params.keys():
                for n, param in enumerate(cfg.model_params["parameter_estimation"]):
                    print(
                        "{}. {} Loss = {}".format(
                            n + 2, param, epoch_training_loss[param]
                        )
                    )
            print(
                "----------------------------------------------------------------------"
    ...
    try:
        ...
        src_best_diagonals = os.path.join(
            export_dir, "DIAGONAL/epoch_{}".format(best_epoch)
        )
        dst_best_diagonals = os.path.join(
            best_dir, "diagonal_epoch_{}".format(best_epoch)
        )
        copy_tree(src_best_diagonals, dst_best_diagonals)
        # Learn params
        src_best_learn = os.path.join(
            export_dir, "LEARN_PARAMS/epoch_{}".format(best_epoch)
        )
        dst_best_learn = os.path.join(
            best_dir, "learn_params_epoch_{}".format(best_epoch)
        )
        ...
        weights_file_ext = split_weights_path[1]
        weights_path = "{}_{}{}".format(
            weights_root_name, cfg.save_best_option, weights_file_ext
        )
