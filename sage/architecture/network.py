...
from sage.architecture.zoo.dain import DAIN_Layer
...
class Rigatoni_MS_ResNetCBAM_legacy_singledet(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Pretrained model is for 3-channels. We use 2 channels.
        if resnet_size == 50:
            self.frontend_1 = resnet50_cbam(
                pretrained=False, num_classes=256, in_channels=1
            )
            self.frontend_2 = resnet50_cbam(
                pretrained=False, num_classes=256, in_channels=1
            )
        elif resnet_size == 152:
