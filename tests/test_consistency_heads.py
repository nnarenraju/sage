...
def test_perdethead_shapes_and_sigma_clamp():
    head = PerDetHead(C, sigma_min=1e-3, sigma_max=10.0)
    ...
    for s in (out.sigma_tc, out.sigma_mc):
        assert (s >= 1e-3 - 1e-6).all() and (s <= 10.0 + 1e-4).all()
...
def _mk(mu_tc, sigma_tc, mu_mc=0.0, sigma_mc=1.0):
    n = mu_tc.shape[0]
    return PerDetOutput(
        mu_tc, torch.full((n,), sigma_tc), torch.full((n,), mu_mc),
        torch.full((n,), sigma_mc), torch.zeros(n), torch.zeros(n),
    )


def test_consistency_statistic_is_uncertainty_weighted():
    ltt = torch.tensor(0.0100128)  # H1-L1
    # sigma values (std, not log): exp(-3) ~ 0.05 confident, 1.0 uncertain.
    CONF, FAINT = 0.05, 1.0
    # agreement within the light-travel time -> exactly zero
    s_tc, _ = consistency_statistic(_mk(torch.tensor([11.0]), CONF),
                                    _mk(torch.tensor([11.005]), CONF), ltt)
    assert float(s_tc) < 1e-6
    # large disagreement, both confident -> large statistic
    s_conf, _ = consistency_statistic(_mk(torch.tensor([11.0]), CONF),
                                      _mk(torch.tensor([11.1]), CONF), ltt)
    # same disagreement but one detector uncertain (large sigma) -> tolerated
    s_faint, _ = consistency_statistic(_mk(torch.tensor([11.0]), CONF),
                                       _mk(torch.tensor([11.1]), FAINT), ltt)
...
def test_corroboration_features_shape():
    a = _mk(torch.zeros(B), 0.37)
    b = _mk(torch.zeros(B), 0.37)
...
def test_consistency_loss_masking_and_grad():
    ...
    sigma = torch.ones(B, D, requires_grad=True)  # sigma=1 -> log(sigma)=0
    y = torch.zeros(B, D)
    # perfect mean, sigma=1 -> nll = 0.5*0 + log(1) = 0
    out = loss(mu, sigma, mu, sigma, y, y.mean(1), torch.ones(B, D))
    assert torch.allclose(out, torch.zeros(3), atol=1e-6)
    # fully-masked-out batch -> zero loss regardless of error
    out0 = loss(mu + 5, sigma, mu + 5, sigma, y, y.mean(1), torch.zeros(B, D))
