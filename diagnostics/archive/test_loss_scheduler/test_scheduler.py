...
Created on Tue May  3 11:15:31 2022
...
Documentation:
    ...
    # Schedulers
    scheduler_1 = ExponentialGrowth(saturate_within=20, start_value=0.0001, end_value=0.2)
    scheduler_2 = ExponentialGrowth(saturate_within=20, start_value=0.2, end_value=0.9)
    # Apply functions
...
class ExponentialGrowth:
    ...
    def __init__(self, saturate_within=5, start_value=0.01, end_value=1.0):
        assert start_value < end_value
        self.saturate_within = saturate_within
        self.start_value = start_value
        self.end_value = end_value
        
        # Estimated growth rate for the given number of epochs and start_value
        self.growth_rate = (self.end_value/self.start_value)**(1./(self.saturate_within-1)) - 1.0
    ...
    def __call__(self, scheduler_vals=None):
        ...
        if not isinstance(scheduler_vals, np.ndarray):
            return self.exp_growth(np.arange(self.saturate_within))
        else:
            return np.append(scheduler_vals, self.exp_growth(np.arange(self.saturate_within)))
