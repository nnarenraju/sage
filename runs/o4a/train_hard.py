...
# -*- coding: utf-8 -*-
"""
O4a BBH hard negative mining training.

Pipeline
--------
Signal  : IMRPhenomPv2, time-domain multirate sampling
Noise   : MemmapNoiseSampler pooling O3a + O3b (multi-run)
          + RecolourPostprocess recolouring toward O4a (the eval run)
...
logger = get_logger("sage.run.o4a.hard")
...
def make_training_graph(seed):
    ...
    recolour = RecolourPostprocess(
        p_recolour          = 0.37,
        recolour_dataset_dir= f"{get_server().data_root}/data_release_o4a",  # -> O4a
    )
