...
"""
Filename        : dataset.py
Description     : O4a noise download + ASD generation.

O4a public data covers H1 and L1 only (Virgo joined for O4b). Everything is
written into its OWN release sub-folder (data_release_o4a) so it can never
...
_RUN = "O4a"
_DETECTORS = ["H1", "L1"]  # O4a: H1 + L1 only (no Virgo)
...
# Keep O4a in its OWN release sub-folder, completely separate from every other
# run's directory, so concurrent downloads can never touch each other.
_RELEASE_DIRNAME = "data_release_o4a"

# The O4a noise .bin files and their *_segments.json sidecars live here
...
def make_asds_only():
    ...
    resolves to runs/o4a/run_export for the fiducial ASDs.
    """
    set_configs()
