...
def read_data(args, idxs):
    ...
    print('Dataset {} comparing {} against {}'.format(args.dataset, team_1['name'], team_2['name']))
...
def compare_plot_1(team_1, team_2, save_dir):
    ...
    for thresh_name in thresh_names:
        ...
        for param, (i, j)  in zip(params, pidxs):
            ...
            bins=np.histogram(np.hstack((team_2[param][team_2[thresh_name]], team_1[param][team_1[thresh_name]])), bins=64)[1] #get the bin edges
            ax[i][j].hist(team_2[param][team_2[thresh_name]], bins=bins, label=team_2['name'], color='red', histtype='stepfilled', alpha=0.5)
            ax[i][j].hist(team_1[param][team_1[thresh_name]], bins=bins, label=team_1['name'], color='blue', **kwargs)
...
def main(raw_args=None, cfg_far_scaling_factor=None, dataset=None):
    ...
    with h5py.File('/home/nnarenraju/Research/ORChiD/results/{}/ds{}/eval.hdf'.format(args.team2, dataset)) as fp:
