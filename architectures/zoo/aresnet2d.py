...
from architectures.zoo.attention import AttentionConv, AttentionStem
...
class Model(nn.Module):
    def __init__(self, block, num_blocks, num_classes=1, stem=False):
        super(Model, self).__init__()
        self.in_places = 64

        if stem:
            self.init = nn.Sequential(
                # For KaggleNet Frontend
                AttentionStem(in_channels=2, out_channels=64, kernel_size=4, stride=1, padding=2, groups=1),
                nn.BatchNorm2d(64),
                nn.ReLU(),
                nn.MaxPool2d(4, 4)
            )
